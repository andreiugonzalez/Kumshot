...
import { X, Link2, FileText, AlertTriangle, CheckCircle, Pencil, Info } from 'lucide-react';
...
export default function EditLinkModal({ isOpen, onClose, link, onUpdated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const result = await updateLink(link.id, { title: title.trim(), url: url.trim() });
    setLoading(false);
          ...
          <div className="mb-4 pt-1">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-8 h-8 md:w-10 md:h-10 bg-accent flex items-center justify-center border-2 border-black shadow-[4px_4px_0px_#000]">
                <Pencil size={18} className="md:w-5 md:h-5 text-black" />
              </div>
...
{RULES_HINT}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => navigator.clipboard?.writeText(RULES_HINT)}
